...
> `VISION.md`, `suite CONTEXT.md`,
> `PLATFORM-REFERENCE.md`, `ARCHITECTURE-DECISIONS.md`, and
> `BOUNDARIES.md` rather than redefining anything already
...
background-automation plugins: `ARCHITECTURE-DECISIONS.md` ADR-029.)*
...
bigger plugin. See `suite CONTEXT.md`'s "MVP Bulk Create" entry and
`ARCHITECTURE-DECISIONS.md` ADR-029/ADR-010 for the full reasoning
behind this being the confirmed v1 scope. For Plugin 2's ownership
boundaries, see `BOUNDARIES.md` §Build Order Generator.
...
- Classifies every node as **Existing** (real child BO already exists) or
  ...
  *(`suite CONTEXT.md`, "Existing Node" / "Draft Node")*
- Computes each node's **Outstanding** quantity per `suite CONTEXT.md`:
  ...
  hint, never filtering the number itself *(`suite CONTEXT.md`, "Outstanding")*
- **Merges duplicate sub-assemblies** appearing in multiple branches into
  one canonical node, combined by default *(`suite CONTEXT.md`,
...
- Renders **collapsed by default with rollup badges** (e.g. "4 need
  ...
  every branch *(`suite CONTEXT.md`, "Rollup Badge")*
...
- **Refuses to create a child BO unless its parent is explicit** —
  ...
  parent-before-child creation) *(`suite CONTEXT.md`, "Creation Order
  Constraint")*
- A **"Show build candidates only" filter** — hides leaf nodes that
  already have a default supplier set; those are Purchase List Generator's concern, not
  build candidates *(`suite CONTEXT.md`, "Build Candidate" / "Show
...
Parts / Allocate Stock) to cover the rest. See `suite CONTEXT.md`'s
"Commit Decision Model" section for the full enhancement-layer design, and
`ARCHITECTURE-DECISIONS.md` ADR-004, 006, 007, 009, 016, 017 for the
decisions behind it.
...
- LCA-merge interaction with bulk create: don't double-create a combined
  ...
  open questions, `suite CONTEXT.md`)
...
- Depth/node-count threshold for large-tree handling (unresolved,
  `ARCHITECTURE-DECISIONS.md` open items)
- LCA reference-node visual treatment: how a non-expandable reference row
  clearly reads as non-interactive is unresolved *(`suite CONTEXT.md`,
  "Canonical Node / Reference Node")*
